# Editor Setup

All editor configs support the same server settings.

## Full settings schema

```json [settings.json]
{
  "solidity-language-server": {
    "inlayHints": {
      "parameters": true,
      "gasEstimates": true
    },
    "lint": {
      "enabled": true,
      "severity": [],
      "only": [],
      "exclude": []
    },
    "fileOperations": {
      "templateOnCreate": true,
      "updateImportsOnRename": true,
      "updateImportsOnDelete": true
    },
    "projectIndex": {
      "fullProjectScan": false,
      "cacheMode": "auto",
      "incrementalEditReindex": false,
      "incrementalEditReindexThreshold": 0.4
    }
  }
}
```

## Lint values

Foundry lint config reference (lint_on_build): [Foundry linter config docs](https://www.getfoundry.sh/config/reference/linter#lint_on_build)

- `lint.severity`: `"high"`, `"med"`, `"low"`, `"info"`, `"gas"`, `"code-size"`
- `lint.only` / `lint.exclude` rule IDs:
  - `incorrect-shift`
  - `unchecked-call`
  - `erc20-unchecked-transfer`
  - `divide-before-multiply`
  - `unsafe-typecast`
  - `pascal-case-struct`
  - `mixed-case-function`
  - `mixed-case-variable`
  - `screaming-snake-case-const`
  - `screaming-snake-case-immutable`
  - `unused-import`
  - `unaliased-plain-import`
  - `named-struct-fields`
  - `unsafe-cheatcode`
  - `asm-keccak256`
  - `custom-errors`
  - `unwrapped-modifier-logic`

## Notes

- Empty arrays for `severity`, `only`, `exclude` mean "no filter".
- Defaults are all enabled (`true`) with empty lint arrays.
- `projectIndex.fullProjectScan` defaults to `false` to keep startup latency low.
- `projectIndex.cacheMode` controls persistent cache strategy:
  - `"auto"` (default): try cache v2 first, fallback to v1.
  - `"v1"`: force legacy all-or-nothing cache.
  - `"v2"`: force per-file shard cache.
- `projectIndex.incrementalEditReindex` enables an aggressive dependency-closure
  scoped reindex path on dirty sync (falls back to full reindex on failure).
- `projectIndex.incrementalEditReindexThreshold` (default `0.4`) controls when
  scoped reindex is allowed: if `affected_sources / total_sources` exceeds this
  ratio, the server skips scoped reindex and runs full reindex.
- `projectIndex.fullProjectScan` is available only in versions newer than `v0.1.26` (`v0.1.27+`).
- In `v0.1.27+`, on-disk project cache uses a single-flight `didSave` sync after file operations (`didCreateFiles` / `didRenameFiles` / `didDeleteFiles`) to avoid overlapping re-index jobs during save bursts.
- For file rename workflows, editors may require explicit save-all to persist buffer edits to disk.
