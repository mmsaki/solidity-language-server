...
- **Project Indexing Mode** — full-project indexing via `projectIndex.fullProjectScan` (enabled by default), persistent cache selection via `projectIndex.cacheMode` (`auto` / `v1` / `v2`), and aggressive dependency-closure dirty-sync via `projectIndex.incrementalEditReindex`
